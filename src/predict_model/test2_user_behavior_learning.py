...
def create_model(alpha: float = 0.1, motivation_bias: float = 0.0) -> MultiFeatureLinUCB:
...
def get_features(motivation: int, duration: int, difficulty: int, 
                 hours_to_deadline: float, category: str) -> np.ndarray:
...
def test_high_performer():
    ...
    for i, (mot, dur, diff, hours, cat, reward) in enumerate(training_tasks):
        ...
        print(f"  Task {i+1}: {cat:6} (diff={diff}) | Reward={reward:.2f} | "
...
def test_pressure_performer():
    ...
    for i, (mot, dur, diff, hours, cat, reward) in enumerate(training_tasks):
        ...
        if hours <= 6:
            ...
            pressure = "NONE"
        print(f"  Task {i+1}: {pressure:6} ({hours:3}h to deadline) | Reward={reward:.2f}")
...
def test_inconsistent_user():
    ...
    training_rewards = [0.90, 0.15, 0.85, 0.20, 0.10, 0.95, 0.25, 0.80, 
                        0.15, 0.90, 0.30, 0.85, 0.20, 0.92, 0.18, 0.88]
    ...
    for i, reward in enumerate(training_rewards):
        ...
        print(f"  Task {i+1:2}: Reward={reward:.2f} | Score: {score_before:.3f} → {score_after:.3f}")
    ...
    test_passed = abs(final_score - avg_reward) < 0.15  # Score should be close to average
