...
const CreateTask: React.FC = () => {
  ...
  const categoryOptions = useMemo(
    () => CATEGORY_KEYS.map((key) => CATEGORY_META[key]?.displayName || key),
    []
  );
   ...
   */
  const handleNumSubtasksChange = useCallback((value: number) => {
    if (value < 1) value = 1;
  ...
  const updateSubtask = useCallback(
    (index: number, field: keyof Subtask, value: any) => {
      setFormState((prev) => {
            ...
            {isCalendarModalVisible && (
              <View style={styles.inlineCalendarContainer}>
                <CalendarPicker
                  ...
                  selectedDate={formState.timeToComplete}
                />
              </View>
            )}
          </View>
