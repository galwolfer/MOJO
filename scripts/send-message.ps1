<#
MOJO CLI - Send Message (v3, ASCII-only)
Usage: .\scripts\send-message.v3.ps1 -Message "Your message"
This script loads a JWT from scripts/.token if -Token is not provided.
#>
...
try {
    ...
    if ($response.widget) {
        Write-Host ""
        Write-Host "Widget data available:" -ForegroundColor Yellow
        Write-Host ($response.widget | ConvertTo-Json -Depth 10) -ForegroundColor Gray
    }
