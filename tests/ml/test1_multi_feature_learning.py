...
def print_results(results):
    ...
    for result in results:
        ...
        print(
            f"  Motivation={result.get('motivation', '-'):<2} "
    ...
    test_cases = [
        {
            ...
            "delta_hours": 100,
            "category": "sport",
            "desc": "Low motivation, short, easy, no pressure, sport",
        },
        {
            "motivation": 3,
            "duration": 60,
            ...
            "delta_hours": 24,
            "category": "study",
            "desc": "Medium motivation, medium, medium, mild pressure, study",
        },
        {
            "motivation": 5,
            "duration": 120,
            ...
            "delta_hours": 2,
            "category": "work",
            "desc": "High motivation, long, hard, urgent, work",
        },
    ]
...
def test_motivation_feature_learning():
    ...
    print("-" * 70)
    print(
        f"{'Init':<8} {'1':<12} {'-':<10} {model_low.predict_score(x_low):<18.6f} {model_low.predict_category(x_low):<12}"
    )
    for i in range(1, 11):
        model_low.update(x_low, reward=0.2)
        print(
            f"{i:<8} {'1':<12} {'0.2':<10} {model_low.predict_score(x_low):<18.6f} {model_low.predict_category(x_low):<12}"
        )
    ...
    print("-" * 70)
    print(
        f"{'Init':<8} {'5':<12} {'-':<10} {model_high.predict_score(x_high):<18.6f} {model_high.predict_category(x_high):<12}"
    )
    for i in range(1, 11):
        model_high.update(x_high, reward=1.0)
        print(
            f"{i:<8} {'5':<12} {'1.0':<10} {model_high.predict_score(x_high):<18.6f} {model_high.predict_category(x_high):<12}"
        )
...
def test_duration_feature_learning():
    ...
    print("-" * 70)
    print(
        f"{'Init':<8} {'10':<12} {'-':<10} {model_short.predict_score(x_short):<18.6f} {model_short.predict_category(x_short):<12}"
    )
    for i in range(1, 11):
        model_short.update(x_short, reward=0.9)
        print(
            f"{i:<8} {'10':<12} {'0.9':<10} {model_short.predict_score(x_short):<18.6f} {model_short.predict_category(x_short):<12}"
        )
    ...
    print("-" * 70)
    print(
        f"{'Init':<8} {'120':<12} {'-':<10} {model_long.predict_score(x_long):<18.6f} {model_long.predict_category(x_long):<12}"
    )
    for i in range(1, 11):
        model_long.update(x_long, reward=0.3)
        print(
            f"{i:<8} {'120':<12} {'0.3':<10} {model_long.predict_score(x_long):<18.6f} {model_long.predict_category(x_long):<12}"
        )
...
def test_difficulty_feature_learning():
    ...
    print("-" * 70)
    print(
        f"{'Init':<8} {'1':<12} {'-':<10} {model_easy.predict_score(x_easy):<18.6f} {model_easy.predict_category(x_easy):<12}"
    )
    for i in range(1, 11):
        model_easy.update(x_easy, reward=0.95)
        print(
            f"{i:<8} {'1':<12} {'0.95':<10} {model_easy.predict_score(x_easy):<18.6f} {model_easy.predict_category(x_easy):<12}"
        )
    ...
    print("-" * 70)
    print(
        f"{'Init':<8} {'3':<12} {'-':<10} {model_med.predict_score(x_med):<18.6f} {model_med.predict_category(x_med):<12}"
    )
    for i in range(1, 11):
        model_med.update(x_med, reward=0.5)
        print(
            f"{i:<8} {'3':<12} {'0.5':<10} {model_med.predict_score(x_med):<18.6f} {model_med.predict_category(x_med):<12}"
        )
    ...
    for i in range(1, 11):
        ...
        print(
            f"{i:<8} {'5':<12} {'0.2':<10} {model_hard.predict_score(x_hard):<18.6f} {model_hard.predict_category(x_hard):<12}"
        )
...
def test_pressure_feature_learning():
    ...
    print("-" * 70)
    print(
        f"{'Init':<8} {'100':<8} {'-':<10} {model_no.predict_score(x_no_pressure):<18.6f} {model_no.predict_category(x_no_pressure):<12}"
    )
    for i in range(1, 11):
        model_no.update(x_no_pressure, reward=0.8)
        print(
            f"{i:<8} {'100':<8} {'0.8':<10} {model_no.predict_score(x_no_pressure):<18.6f} {model_no.predict_category(x_no_pressure):<12}"
        )
    ...
    print("-" * 70)
    print(
        f"{'Init':<8} {'2':<8} {'-':<10} {model_urgent.predict_score(x_urgent):<18.6f} {model_urgent.predict_category(x_urgent):<12}"
    )
    for i in range(1, 11):
        model_urgent.update(x_urgent, reward=0.3)
        print(
            f"{i:<8} {'2':<8} {'0.3':<10} {model_urgent.predict_score(x_urgent):<18.6f} {model_urgent.predict_category(x_urgent):<12}"
        )
...
def test_category_feature_learning():
    ...
    for i in range(1, 7):
        ...
        print(
            f"{i:<8} {'sport':<12} {'0.9':<10} {model_sport.predict_score(x_sport):<18.6f} {model_sport.predict_category(x_sport):<12}"
        )
    ...
    for i in range(1, 7):
        ...
        print(
            f"{i:<8} {'study':<12} {'0.5':<10} {model_study.predict_score(x_study):<18.6f} {model_study.predict_category(x_study):<12}"
        )
    ...
    for i in range(1, 7):
        ...
        print(
            f"{i:<8} {'work':<12} {'0.3':<10} {model_work.predict_score(x_work):<18.6f} {model_work.predict_category(x_work):<12}"
        )
...
def test_combined_learning():
    ...
    training_data = [
        {
            ...
            "category": "sport",
            "reward": 0.95,
            "desc": "Easy sport task, high motivation, no pressure",
        },
        {
            "motivation": 4,
            "duration": 30,
            ...
            "category": "study",
            "reward": 0.7,
            "desc": "Medium study task, good motivation",
        },
        {
            "motivation": 2,
            "duration": 100,
            ...
            "category": "work",
            "reward": 0.1,
            "desc": "Difficult work task, low motivation, urgent",
        },
        {
            "motivation": 5,
            "duration": 20,
            ...
            "category": "health",
            "reward": 0.9,
            "desc": "Easy health task, high motivation",
        },
        {
            "motivation": 3,
            "duration": 60,
        ...
        },
        {
            ...
            "category": "habits",
            "reward": 0.85,
            "desc": "Easy habit, high motivation",
        },
        {
            "motivation": 1,
            "duration": 120,
            ...
            "category": "work",
            "reward": 0.05,
            "desc": "Very hard, low motivation, urgent",
        },
        {
            "motivation": 5,
            "duration": 5,
        ...
        },
        {
            ...
            "category": "health",
            "reward": 0.7,
            "desc": "Medium health task with some pressure",
        },
    ]
    ...
    # Initial theta
    print(
        f"{'Init':<8} {'-':<12} {'-':<10} {'-':<12} {'-':<10} {np.mean(model.theta):<12.6f} {np.var(model.theta):<15.6f}"
    )
    ...
    for epoch in range(2):
        for i, task in enumerate(training_data):
            ...
            if (update_num) % 5 == 0:  # Print every 5 updates
                print(
                    f"{update_num:<8} {task['motivation']:<12} {task['category']:<10} {task['difficulty']:<12} {task['reward']:<10} "
    ...
    test_cases = [
        {
            ...
            "delta_hours": 100,
            "category": "sport",
            "label": "Ideal task (easy, motivated, no pressure)",
        },
        {
            "motivation": 3,
            "duration": 60,
            ...
            "delta_hours": 24,
            "category": "study",
            "label": "Typical task (medium everything)",
        },
        {
            "motivation": 1,
            "duration": 120,
            ...
            "delta_hours": 2,
            "category": "work",
            "label": "Worst task (hard, unmotivated, urgent)",
        },
    ]
...
def test_user_behavior_progression():
    ...
    print_header(
        "TEST 8A-C: USER BEHAVIOR PROGRESSION WITH FIXED FEATURES (vary rewards)"
    )
    print(
        "\nSingle task features (fixed): motivation=2, duration=90, difficulty=4, pressure=12h, category=work"
    )
    print(
        "We simulate the same user/task over time; the user's performance (reward) improves over phases."
    )

    # Use a single feature vector for the entire progression
    x_user = extract_features(
        motivation=2,
        duration=90,
        difficulty=4,
        delta_hours=12,
    ...
    print(
        f"{'Init':<8} {'POOR':<12} {'-':<10} {model.predict_score(x_user):<18.6f} {model.predict_category(x_user):<12}"
    )
    for i in range(1, 6):
        model.update(x_user, reward=0.1)
        print(
            f"{i:<8} {'POOR':<12} {'0.1':<10} {model.predict_score(x_user):<18.6f} {model.predict_category(x_user):<12}"
        )
    ...
    for i in range(6, 11):
        ...
        print(
            f"{i:<8} {'MEDIUM':<12} {'0.5':<10} {model.predict_score(x_user):<18.6f} {model.predict_category(x_user):<12}"
        )
    ...
    for i in range(11, 16):
        ...
        print(
            f"{i:<8} {'EXCELLENT':<12} {'0.8':<10} {model.predict_score(x_user):<18.6f} {model.predict_category(x_user):<12}"
        )
    ...
    for offset, r in enumerate(rewards):
        ...
        print(
            f"{idx:<8} {'FLUX':<12} {r:<10} {model.predict_score(x_user):<18.6f} {model.predict_category(x_user):<12}"
        )
