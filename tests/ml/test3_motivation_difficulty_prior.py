...
def test_learning_preserves_prior_influence():
    ...
    for i in range(1, 16):
        ...
        note = (
            "Blending prior + data"
            if i < 5
            else "Data dominates" if i > 10 else "Transitioning"
        )
...
def test_different_difficulty_levels():
    ...
    print(
        f"{'Init':<8} {score_easy:<14.4f} {score_hard:<14.4f} {score_easy - score_hard:<14.4f}"
    )
    ...
    for i in range(1, 11):
        ...
        score_hard = model_hard.predict_score(x_hard)
        print(
            f"{i:<8} {score_easy:<14.4f} {score_hard:<14.4f} {score_easy - score_hard:<14.4f}"
        )
...
def test_motivation_impact():
    ...
    print(
        f"{'Update':<8} {'Low Mot Score':<16} {'High Mot Score':<16} {'Difference':<14}"
    )
    print("-" * 65)
    ...
    print(
        f"{'Init':<8} {score_low:<16.4f} {score_high:<16.4f} {score_high - score_low:<14.4f}"
    )
    ...
    for i in range(1, 11):
        ...
        score_high = model_high.predict_score(x_high)
        print(
            f"{i:<8} {score_low:<16.4f} {score_high:<16.4f} {score_high - score_low:<14.4f}"
        )
...
def test_combined_prior_effect():
    ...
    print(
        "(Score = motivation_weight * mot_norm + difficulty_weight + exploration_bonus)"
    )
    print()
...
def test_real_world_scenario():
    ...
    print(
        f"{'#':<4} {'Description':<25} {'Pred Score':<12} {'Pred Cat':<10} {'Reward':<8} {'Post Score':<12}"
    )
    print("-" * 85)
    ...
    for i, task in enumerate(tasks, 1):
        ...
        print(
            f"{i:<4} {task['desc']:<25} {pre_score:<12.4f} {pre_cat:<10} {task['reward']:<8} {post_score:<12.4f}"
        )
