...
async function test2_userSetup() {
  ...
  if (!testUser) {
    ...
    console.log(`   🧹 Cleaned up ${deletedTasks.deletedCount} old tasks and ${deletedSchedules.deletedCount} schedules`);
  }
...
async function test3_taskCreation() {
  ...
  console.log(`       ML Prediction: score=${task1.predictionScore?.toFixed(3)}, category=${task1.predictedCompletionCategory}`);
  ...
  console.log(`       ML Prediction: score=${task2.predictionScore?.toFixed(3)}, category=${task2.predictedCompletionCategory}`);
  ...
  console.log(`       ML Prediction: score=${task3.predictionScore?.toFixed(3)}, category=${task3.predictedCompletionCategory}`);
...
async function test5_taskCompletion(tasks) {
  ...
  console.log(`       Difference: ${result1.actualCompletionMinutes - task1.estimatedDuration} min (${result1.actualCompletionMinutes < task1.estimatedDuration ? '✅ faster' : '⚠️ slower'})`);

  // Calculate reward manually to show
  const reward1 = calculateReward(result1.actualCompletionMinutes, task1.estimatedDuration);
  console.log(`       Reward: ${reward1.toFixed(3)} (${reward1 > 0.7 ? '✅ good' : reward1 > 0.5 ? '⚠️ okay' : '❌ poor'})`);
  ...
  console.log(`       Difference: ${result2.actualCompletionMinutes - task2.estimatedDuration} min (${result2.actualCompletionMinutes < task2.estimatedDuration ? '✅ faster' : '⚠️ slower'})`);

  const reward2 = calculateReward(result2.actualCompletionMinutes, task2.estimatedDuration);
  console.log(`       Reward: ${reward2.toFixed(3)} (${reward2 > 0.7 ? '✅ good' : reward2 > 0.5 ? '⚠️ okay' : '❌ poor'})`);
...
async function test6_verifyLearning(tasks, completionResults) {
  ...
  console.log(`       Change: ${(similarTask1.predictionScore - task1.predictionScore).toFixed(3)} (${Math.abs(similarTask1.predictionScore - task1.predictionScore) > 0.01 ? '✅ learned!' : '→ stable'})`);
  ...
  console.log(`       Change: ${(similarTask2.predictionScore - task2.predictionScore).toFixed(3)} (${Math.abs(similarTask2.predictionScore - task2.predictionScore) > 0.01 ? '✅ learned!' : '→ stable'})`);
...
async function test7_priorityScoring() {
  ...
  result.queue.forEach((task, index) => {
    ...
    if (task.mlPrediction) {
      ...
      console.log(`      ML: confidence=${task.mlPrediction.confidence.toFixed(2)}, category=${task.mlPrediction.category}, weight=${task.mlPrediction.weight.toFixed(2)} ${boost}`);
    } else {
...
async function test9_modelIsolation() {
  ...
  console.log(`       User 1 prediction: ${taskU1.predictionScore?.toFixed(3)}, category ${taskU1.predictedCompletionCategory}`);
  console.log(`       User 2 prediction: ${taskU2.predictionScore?.toFixed(3)}, category ${taskU2.predictedCompletionCategory}`);
  ...
  console.log(`\n   📊 Model Files:`);
  console.log(`       User 1: ${model1 ? 'exists' : 'not created'} ${model1 ? `(${(model1.size / 1024).toFixed(2)} KB)` : ''}`);
  console.log(`       User 2: ${model2 ? 'exists' : 'not created'} ${model2 ? `(${(model2.size / 1024).toFixed(2)} KB)` : ''}`);
